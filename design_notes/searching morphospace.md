...
* The more abstract a layer (the number of layers below it), the longer the timeframes of its dCAS and the lesser their sensitivity to stress.

* The first dCA on a new layer has the entire layer below as its umwelt (it is a "stem" dCA).

* A dCA replicates (makes an identical copy of itself), if under enough stress, until a replication limit is reached (too many "replicant" dCAs in the layer vs the size of the layer below).

* A dCA that has stopped replicating switches to division (it becomes "mitotic") unless it is already "mature" (a mature dCA has a small umwelt that can not be further divided).

* A mitotic dCA splits into 2 "daughter" dCAs if under stress. It splits at a "joint", dividing its umwelt and cognitive state where possible. Causal theories and some elements of its cognitive state may be lost because of the partition. A "joint" is a way of dividing a dCA into two that minimizes loss.

* The "daughter" dCAs born from division each replicate under stress until they reach the replication limit and become mitotic, unless they are already "mature" (they are then "end-of-line" dCAs).

* "Replicant" dCAs and "daughter" dCAs differentiate over time because of non-deterministic causal theory generation but also from non-determinism in making predictions, unifying experiences, making plans, etc.

* A dCA remembers its lineage (the sequence of replications and divisions that led to it from the "stem" dCA in its layer) even though it differentiates. This allows counting toward replication limits.
