...
So, a Cognition Actor (CA) acts by giving itself a goal (its intent) and by being given goals (directives), and then finding plans that might achieve some or all of them, and by executing these plans.
...
An executable plan is found only by a CA when, for each of the plan's directives, an executable plan is found by the CA's umwelt.
...
A **goal** is a relation/property experienced by the CA (the goal is an intent) or experienced by its umwelt (the goal is a directive), that a CA aims to impact in a certain way.

An *intent* is a self-assigned goal of the CA to impact a felt experience it has.

A *directive* is a goal delegated by a CA to its umwelt CAs requesting that they impact experiences that the CA observed them having and from which it synthsized its own felt experiences.
...
Achieving a goal and the planned sub-goals it depends on requires coordination between a parent CA and its umwelt CAs. During any phase of its lifecycle, a CA receives events and messages regarding the status of goals and plans. An event is multicasted by a CA to its umwelt or parent CAs, whereas a message targets a single CA.
...
* Advances goals (its intent or directives it received) toward completion, as urgency dictates
...
* Determines the success or failure of previously executed plans
  * Plans built by the CA and executed are given a score (or their score is updated) from an assessment of their success, making them more or less attractive affordances

### Communications

A directive (goal sent or received) is communicated by value or by reference.
...
It is communicated by value to parents because some parents may not yet have been requested to do it but might.

An intent (self-assigned goal) is always communicated by reference (an umwelt does not need to know the nature of the intents of its ancestors, only their identity).

CAs always communicate about goals with umwelt CAs via events so as to reach all umwelt CAs.
Umwelt CAs also always communicate about goals with parents via events (multicasts), so they all update goal states for directives they have sent or might send.

A CA communicates plans to its umwelt CAs via events. Umwelt CAs send confirmations about plans to parents who sent them via messages since this is of no interest to other parents.
...
* For each directive,
  * if not relevant to the umwelt CA, broadcast `cannot_seek(Directive)` to all parents
  * if relevant, broadcast `can_seek(Directive)`

##### Event `find_plan([directive_id=DirectiveId])`

Once it has received feedback from (enough) of its umwelt about a plan it built and disseminated, a CA asks its umwelt CAs to construct, with some priority, a plan to achieve a directive from its own plan, in the context of an intent (its own or that of an ancestor CA). Only the umwelt CAs who can_seek the directive work on a plan, the others ignore the broadcasted request. The plan can be executed only if there is an executable umwelt plan for each of its directives.

* If an executable plan is found for a directive received
...
A CA asks its umwelt CAs to execute any plan they found for a directive sent to them by the CA.
...
* Else
  * For each directive in the plan the umwelt CA built
  ...
  * Once completed, send the parent CA confirmation event that the plan for the received directive was executed
...
A CA tells its parent CAs, in response to a parent broadcasting `todo` directives to its umwelt, that it might be able to impact a given directive as requested, i.e. it refers to one of its experiences.

##### Event `cannot_seek([directive=Directive])`

A CA tells its parent CAs, in response to a parent broadcasting a `todo` directives to its umwelt, that a given directive is not recognized, i.e. it does **not** refer to one of its experiences.

##### Event `can_execute([directive=Directive])`

A CA tells its parent CAs, in response to a `find_plan` event broadcasted a parent, that it successfully built such a plan, refering to by its unique id, for a directive it received.
...
A parent CA's plan (at level 1) are composite actions (e.g. [left_wheel:spin, left_wheel:spin, right_wheel:reverse_spin]). Each action is already known by the parent CA to be meaningful for at least one CA in its umwelt.

Being composite actions, such plans are executed at once instead of as a sequence of directives. Execution is assumed to always succeed (even if a wheel is stuck in the mud etc.).
...
* If an effector CA is asked by a parent to prepare for N identical actions for an intent and then by another to prepare for N + M actions for the same intent
  * It accumulates N + M, not N + N + M in the context of the intent
* The effector CA messages `actions_received` directly back to the parent CA

##### Event `ready_actuations([plan_id=PlanId, intent_id:IntentId])`

A level 1 CA tells its umwelt effector CAs to ready the body to realize the actions it associated with a plan.
...
* The effector CA forgets all actuations associated with the plan
...
* When all effector CAs have confirmed receipt of actions for the intent from a parent (whether or not any or all actions are meaningful to an effector CA)
...
* When all effector CAs have confirmed to the parent that actuations in the context of an intent are ready
  * the parent sends `execute` to the `body` (wheels are then spun etc.)
...
stateDiagram-v2
  ...
  unknown --> possible : the plan might be executable
