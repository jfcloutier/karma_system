...
A CA builds a plan when it has identified a current experience it wants to have, persist or terminate (to realize its intent), or when it receives a prediction from a parent CA about it impacting an experience it may have (to realize a parent's directive). When receiving directives (as predictions) from parents, a CA determines which received are relevant to it; the others are likely relevant to sibling CAs participating in the same umwelt.

Intents and directives are goals. Goals name umwelt experiences and the desired impacts (create, persist or terminate). It is *entirely* up to the goal-receiving umwelt CA what plan it constructs given a goal to achieve.

The composition of the plan depends on the type of experience and on the desired impact (create/persist/terminate). The plan's priority depends on the CA's feeling (wellbeing value and gradient) if originating with the CA, or on the priority of the received directive.

How to impact an experience depends on how it was arrived at. A constructed (aka synthetic) experience is composed of observations (as evidence) which are themselves umwelt experiences the CA observed. To impact an experience a CA holds, the CA needs to impact experiences it observed as held in its umwelt.
...
The task of constructing a plan -in order to impact an experience held by the CA- consists in deciding which umwelt experiences to impact and how.
...
The causal theory, a model of the generative processes causing these observations, if present, informs the CA in the construction of hopefully effective plans by inferring their consequences.

To recap, the CA can seek to impact an experience it holds, or create a new one, by impacting causative experiences held in its umwelt.
This is achieved by the CA directing its umwelt (via predictions) to find a way to impact their experiences in prescribed ways (create vs persist vs terminate).

The only types of experiences that can be impacted are `count`, `more`, and `trend` experiences. A sensing experience (e.g. distance(ir_sensor_1, 10)) is a record of a sensor reading that was taken. There is no point denying it (the CA might still want to validate it though). The same applies to action experiences (e.g. spin(motor_1, true)). There is also no point in denying it happened. An imagined experience is one abduced by the Apperception Engine to unify a causal theory. It must be accepted as "real" for as long as the causal theory is used.
...
A `count` experience encapsulates how many of a given kind of experiences observed in its umwelt there are (for e.g., this motor was spun twice).
...
To persist a more experience, a CA's plan would direct its umwelt CAs to persist both compared counts. It could also create or terminate experiences in equal amounts on both sides of the ledger.
...
* steady
  * to disrupt it, cause a value for the previously trending experience to change
