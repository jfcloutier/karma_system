# Latent elements and causal theories

A CA's observations are predictions it makes about the *experiences* of its umwlet CAs. These predictions may be value-corrected by prediction errors from the umwelt. The predictions are the product of a causal theory held by the CA and induced from its prior observations. Predictions are produced by giving the causal theory current observations as input.

A CA synthesizes its past and present observations into experiences. An experience, for example, could be a trend detected in the observations, such as a shortening of distances observed. Each experience is thus supported by a subset of the CA's observations. Experiences abstract observations.

A causal theory may require more than the observations actually made by the CA to make sense of them. It may need to imagine/invent hidden and unobserved, i.e. **latent**, objects, properties and relations to formulate simple and adequate rules that account for the hidden process behind the observations.
...
* Latent observations predicted by a causal theory can not and need not be validated by umwelt CAs
* A CA might adopt a new causal theory at any time; this new causal theory might have different or no latent elements
* Past observations containing latent elements not underwritten by the current causal theory are ignored (but not erased) when predicting incoming observations or synthesizing the current set of experiences
* Past causal theories are remembered (not indefinitely) and compete with newly obtained causal theories
...
* When relaxing vocabulary constraints on candidate causal theories, the least used latent element is removed first
