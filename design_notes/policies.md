...
A policy is a list of directives, to be communicated to the umwelt CAs. A directive is either a delegated goal (a belief of umwelt CAs to impact) to be achieved by the umwelt however it chooses,
or it is a command, a named and previously executed policy known to the umwelt, to be executed once again by the umwelt.
...
Commands are pre-constructed policies and need be executed only if known. Goals name beliefs and desired impacts and it's up to the receiving CA to construct a policy for each one that's relevant to the CA.
...
The composition of the policy depends on the type of belief and on the desired impact (persist/terminate).
...
Unless it only just got started, a CA will have a causal theory. The CA can use it to predict changes to the beliefs of its umwelt (i.e. incoming observations) as a result of executing particular directives (goals or commands). This can be put to use when shaping a policy.
...
* Causal rules: What is expected to be observed given what was last observed - e.g. A and B then next C
...
or integrating past and current timeframes (`trend`, `started` and `ended` beliefs). A belief can also be a sensor reading (`sensed`) or an `imagined` property/relation (from its causal theory).
...
* By goal: By requesting its umwelt to find a way to impact a belief it holds in a prescribed way
...
How a CA could impact a held belief is further modulated by the kind of belief it is. The only types of beliefs that can be impacted are `count`, `more`, `trend`, `ended` and `started` beliefs.
...
To terminate a more belief, a CA's policy would direct its umwelt CAs to reduce the first counts, or to grow the second count.
...
To further a belief in an ending is simply to prevent the ended observation from being made again (to keep it "ended"), whereas disrupting it is to make the observation again (to "un-end" it).
...
To further a belief in a start is simply to cause an bnservation being made again, whereas disrupting it is to prevent the observation from being made.
...
* a goal: An observed belief plus its desired continuation or termination (the umwelt is free to formulate an appropriate policy)
...
* If it is relevant to the CA
  ...
  * else, for higher-level CAs, the goal is marked "in play" and the CA formulates a policy to realize it (i.e. recursively executes the algorithm)
...
The receiving CA informs the originating CA of the directives are executable, or if none are, that none are.
...
* Else, for each directive in the policy it formulated to achieve the received goal
  * It selects one (preferred?) umwelt CA ready to execute the directive (more than one umwelt CA might be ready, making the others redundant)
