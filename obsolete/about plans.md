...
Only synthetic experiences can be acted on and impacted, namely `count`, `more`, and`trend`. All `activation` experiences and concrete experiences, those from sensor CAs, as well as those imagined by a causal theory, represent "givens" that can not be changed directly by intending to.
